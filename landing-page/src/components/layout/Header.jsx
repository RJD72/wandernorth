...
const links = [["How It Works", "how-it-works"], ["Why Wander North", "why"], ["Features", "features"], ["FAQ", "faq"]];
    ...
    <header className="site-header">
      <div className="shell nav-wrap">
        <a className="brand" href="#top" aria-label="Wander North home">
          <span className="brand-mark" aria-hidden="true">▲</span><span>Wander North</span>
        </a>
        ...
        <nav id="site-navigation" className={open ? "site-nav is-open" : "site-nav"} aria-label="Main navigation">
          {links.map(([label, id]) => <a key={id} href={`#${id}`} onClick={follow}>{label}</a>)}
          <a className="button button--small" href="#waitlist" onClick={() => { follow(); trackEvent("header_cta_clicked"); }}>Join Early Access</a>
        </nav>
      </div>
    </header>
  );
